...
Theorem hoare_asgn_fwd :
  forall (m:nat) (a:aexp_div) (P : Assertion),
  {{~ div_error a /\ P /\ X = m}}
    X := a
  {{ $(fun st => (P (X !-> m ; st)
    /\ st X = aeval_div (X !-> m ; st) a))  }}.
Proof.
  (* FILL IN HERE *) Admitted.
(** [] *)
...
Proof.
  red; intros.
  edestruct H as [st' [EQ Hpost]]; eauto.
Qed.
...
Example while_example :
  ...
  {{X = 3}}.
Proof.
  eapply hoare_consequence_post; [eapply hoare_consequence_pre|].
  - eapply hoare_while with (P:= {{ X <= 3 }}).
    eapply hoare_consequence_pre.
    + eapply hoare_asgn.
    + assertion_auto''.
  - assertion_auto''.
